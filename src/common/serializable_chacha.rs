...
struct SerializableChaCha20Wrapper<'a>(&'a [u8]);
...
impl<'de> Deserialize<'de> for SerializableChaCha20 {
    fn deserialize<D>(deserializer: D) -> Result<SerializableChaCha20, D::Error> where D: Deserializer<'de> {
        ...
        if innards.0.len() != mem::size_of::<SerializableChaCha20>() {
            return Err(serde::de::Error::invalid_length(innards.0.len(), &"mem::size_of::<SerializableChaCha20>()"));
        }
        unsafe {
